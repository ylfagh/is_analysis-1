...
## 调课通知
- 由于学院安排了会计专业技术资格考试，临时占用机房，原定的11周（5月14，5月15日）的课程延后在第14周进行。
...
- 老师现在的实验管理平台中一共有5个实验，每个实验只有一个记分点（0-100分）和对应的文字性的评价。
  对于期末课程设计等较大型的实验不能分知识点精确评价学生的实验情况，实验成绩的主观性较强。
- 需要同学们完善的地方是：
    - 一个老师可以上多门课，每个老师只能维护老师自己的课程及成绩。
    - 一人同学可以上多门课，每个同学只能查询同学自己的课程的实验成绩。
    - 每个实验的实验成绩可以分为多个记分点，每个记分点对应评分标准。
      老师在批改实验的时候，对每个记分点进行评分并输入对应的文字评价，所有记分点的成绩之和为该实验的总成绩。
    - 师生留言系统，通过这个系统，师生之间可以相互交流。
- 期末考核要求
    - 需要同学们完成系统分析与设计，并写成<B>一份</B>详细设计文档。文档不需要写成Word或者Excel，就以Markdown形式加超链接形式，发布到你的GitHUB上。
    - 
    - 文档编写参照实验2至实验5的顺序和要求。
- 评分标准（部分100分）：
